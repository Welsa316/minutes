...
router.get('/', async (req, res, next) => {
  try {
    const { rows: pins } = await query(
      'SELECT * FROM pinned WHERE workspace_id = $1 ORDER BY sort_order ASC, created_at ASC',
      [req.workspaceId],
    );
    ...
    const [c, p, m, n] = await Promise.all([
      byType.client.length ? query(`SELECT id, name FROM clients WHERE id = ANY($1) AND workspace_id = $2 AND deleted_at IS NULL`, [byType.client, ws]) : { rows: [] },
      byType.project.length ? query(`SELECT id, name FROM projects WHERE id = ANY($1) AND workspace_id = $2 AND deleted_at IS NULL`, [byType.project, ws]) : { rows: [] },
      byType.meeting.length ? query(`SELECT id, title FROM meetings WHERE id = ANY($1) AND workspace_id = $2 AND deleted_at IS NULL`, [byType.meeting, ws]) : { rows: [] },
      byType.note.length ? query(`SELECT id, title FROM notes WHERE id = ANY($1) AND workspace_id = $2 AND deleted_at IS NULL`, [byType.note, ws]) : { rows: [] },
    ]);
...
router.post('/', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `INSERT INTO pinned (workspace_id, entity_type, entity_id)
       VALUES ($1, $2, $3)
       ON CONFLICT (entity_type, entity_id) DO NOTHING
       RETURNING *`,
      [req.workspaceId, entity_type, entity_id],
    );
...
router.delete('/:entity_type/:entity_id', async (req, res, next) => {
  try {
    ...
    const r = await query(
      'DELETE FROM pinned WHERE entity_type = $1 AND entity_id = $2 AND workspace_id = $3',
