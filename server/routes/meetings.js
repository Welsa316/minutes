  ...
  try {
    const { rows: mrows } = await query(
      `SELECT m.*,
       ...
       LEFT JOIN clients c  ON c.id = m.client_id  AND c.workspace_id = m.workspace_id
       LEFT JOIN projects p ON p.id = m.project_id AND p.workspace_id = m.workspace_id
       LEFT JOIN ideas r    ON r.id = m.roadmap_id AND r.workspace_id = m.workspace_id AND r.deleted_at IS NULL
       LEFT JOIN entity_tags et ON et.entity_type='meeting' AND et.entity_id=m.id
       LEFT JOIN tags t ON t.id = et.tag_id
       WHERE m.id = $1 AND m.workspace_id = $2
       GROUP BY m.id, c.name, p.name, r.title`,
...
router.post('/', async (req, res, next) => {
  try {
    const { title, subject, client_id, project_id, date, location, pre_notes, live_notes, summary, roadmap_id } = req.body || {};
    ...
    const rid = await scoped('ideas', roadmap_id, req.workspaceId);
    const { rows } = await query(
      `INSERT INTO meetings (workspace_id, title, subject, client_id, project_id, date, location, pre_notes, live_notes, summary, roadmap_id)
...
router.put('/:id', async (req, res, next) => {
  try {
    const {
      ...
      pre_layout, during_layout, after_layout, roadmap_id,
    } = req.body || {};
    const cid = await scoped('clients', client_id, req.workspaceId);
    const pid = await scoped('projects', project_id, req.workspaceId);
    const rid = await scoped('ideas', roadmap_id, req.workspaceId);
    const { rows } = await query(
      ...
      [title, subject, cid, pid, date, location, pre_notes, live_notes, summary,
       pre_layout ?? null, during_layout ?? null, after_layout ?? null, rid, req.params.id, req.workspaceId],
    );
